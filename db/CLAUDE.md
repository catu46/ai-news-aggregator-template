...
just once, in a pool shared by all users) from each person's **taste** (which
lives in per-user votes and deliveries). The practical upshot is that adding more
people to the project does not multiply the curation cost — a post is curated
...
database, run `reset.sql` first (deletes everything) and then `schema.sql`
(recreates it). On a brand-new database `schema.sql` alone is enough. The pgvector
extension is created by `schema.sql` itself via `CREATE EXTENSION IF NOT EXISTS
vector`.
...
- **`schema.sql`** -> The entire database structure (tables, indexes, triggers).
  ...
  - **`users`** -> one row per person in the project. `telegram_user_id` is unique
    and acts as a lock: only that id can vote. `settings` is a free-form JSONB —
    that's where `settings->balance` lives (the user's feed balance preference,
  ...
  - **`posts`** -> the **shared pool**: one row per unique post from any source
    ...
    vote column here, because votes are per-user. Dedup is guaranteed by the
    ...
    `metadata` JSONB (subreddit, score, likes…), the raw `raw_text` (which
    retention prunes for old rejected posts, recording `raw_text_pruned_at`) and
    the `embedding vector(1024)` with a pinned `embedding_model` (if the model
    changes, it can be detected and re-embedded).
  - **`deliveries`** -> what was delivered to each user (defines the scope of the
    feed and recall). Links `user_id` + `post_id` (unique per pair, with `ON DELETE
    CASCADE`), stores the `telegram_message_id` and the `affinity_score`
    (similarity to the user's taste at delivery time, can be null).
  - **`votes`** -> the per-user "Gold Standard": 1 vote per `(user_id, post_id)`,
    ...
    examples (`seed`) or `manual`. It is the basis for semantic recall ("did I
    like something about XPTO?") and the affinity prior (centroid of the
    embeddings of liked posts).
  - **`focus`** -> temporary feed "direction", per user and per bucket
    ...
    `expires_at` null = no expiry. The idea is one active direction per (user,
    bucket); re-directing replaces the previous one.
  ...
  - **Operational notes (at the end of the file, they are comments — not DDL)** ->
    ...
    (with the tip `SET hnsw.ef_search = 100` to improve recall), the affinity
    prior (cold-start gate: only activates with ~20 votes per class) and the
    retention pruning that clears `raw_text` from old rejected posts that nobody
    liked (keeping verdict + metadata + embedding) to keep the 500MB in check.

- **`reset.sql`** -> **WARNING: DESTRUCTIVE. DELETES EVERYTHING.** It runs `DROP
  TABLE ... CASCADE` on the schema's 5 tables (`focus`, `votes`, `deliveries`,
  `posts`, `users`) and drops the associated functions. It's used to wipe the
  database before re-applying `schema.sql`. With real data, running this loses
  everything irreversibly. The pgvector extension is intentionally left standing
  (`schema.sql` recreates it with `IF NOT EXISTS`).
