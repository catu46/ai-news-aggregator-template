...
Do not include secrets in source code, logs, commit messages, issues, screenshots,
or any file tracked by Git. If a secret is accidentally exposed, treat it as
compromised and **rotate the key immediately**.
...
- **Private deploy repository:** the Railway deploy is done **via git** — the
  service builds from what's committed. For the bot to find these configs in
  production, `config/sources.yaml` (and `seeds.yaml`, if you run the seed
  there) **must be committed** in your **private** deploy repository:
  a normal commit, or `git add -f` if you've gitignored them. Alternatively,
  provide the configuration via environment variables instead of a file.

In other words: we do **not** claim that `sources.yaml`/`seeds.yaml` "stay in
`.gitignore` and are never committed." In a **private** deploy repo they're
typically committed on purpose; what you avoid is exposing them in a
**public** repo. Only the **secrets** (`.env`) are never committed, under no
...
**GitHub Security Advisories** (the repository's *Security -> Report a vulnerability* tab), including:
