...
they can return duplicates freely. Each source also isolates its own
failures (one query/account breaking doesn't take down the others or the other sources).

Beyond the fixed targets in `config/sources.yaml`, `pipeline.run_ingestion` **injects the
active `/focus` topics** straight into collection: **news** topics become searches on Reddit
(`searches` param) and on X (added to `searches`), and **repo** topics become queries on
GitHub. In other words, changing the focus in chat actually PULLS new content, it doesn't
just reorder the delivery.
...
- `source_url`, `raw_text` (title + body, this is what becomes the embedding and goes to the curator),
  `author`, `published_at` (datetime, whenever it can be extracted), `metadata` (free-form
  per-platform dict).

## Files

- **`base.py`** → Defines the `IngestionSource` interface (abstract class). It has a
  `name` attribute and the abstract method `async fetch() -> list[IngestedPost]`. It's the only
...
- **`reddit_source.py`** (`RedditSource`) → Collects posts via Reddit's **public RSS/Atom
  ...
  (`sub1+sub2+...`) to grab all the subs in a single request (less chance of rate-limiting).
  (2) **TOPIC search** (`searches` param — e.g. the `/focus` topics): for each topic, it fires
  `/search.rss` in **four variants** — `top` over the day, week, and month windows, plus `hot`
  ("featured") — combining relevance by upvotes across several time scales with freshness. Each
  variant is a separate request and the **database deduplicates** the overlap (the RSS doesn't
  carry the score, since Reddit sorts server-side). It does a GET with `httpx` (async), parses the
  Atom with `feedparser`, and extracts the body text (which comes as HTML) with `BeautifulSoup`.
  Each feed is fetched in isolation (`_fetch_feed`): a feed that fails/throttles becomes an empty
  list and doesn't take down the rest. **Filters to real posts only:** it keeps only entries whose
  link contains `/comments/` (search sometimes returns communities, not posts). Key points:
  Reddit's `.json` endpoint started returning 403 (Jun 2026), hence the `.rss`; the `source_id`
  comes from the Atom id (`t3_abc123` → `abc123`); the subreddit is discovered from each item's
  link (the multireddit feed mixes the subs together).
  *Documented caveat:* a datacenter IP (e.g. Railway) may be blocked — if that happens,
  you can swap in an authenticated backend behind the same interface.
...
- **`x_source.py`** (`XSource`) → Collects from **X/Twitter** in free mode (cookie-based),
  ...
  `search "<query>"` (searches — e.g. the `/focus` topics). **Each search runs across TWO tabs:**
  `-t Latest` (newest) and `-t Top` (most engaged), combining freshness and relevance; the dedup
  by id (`seen`, via `_collect`) covers the overlap between the tabs and between searches and accounts.
  **Cookie-based auth:** the CLI reads `TWITTER_AUTH_TOKEN` and `TWITTER_CT0`; when passed to the
  constructor, they're injected into the subprocess `env` (to run headless on Railway); on the Mac, the
  CLI uses the logged-in browser's cookies. The `twitter` binary is resolved in PATH → venv
  bin → `~/.local/bin`. Robustness: `stdin=DEVNULL` (never hangs waiting for input), timeout, and
  every failure (timeout, non-JSON, returncode != 0, `ok=false`) becomes an empty list with a log, without
  taking down collection. Includes the quoted tweet in `raw_text` as context for the curator. Constructor:
  `accounts`, `searches`, `auth_token`, `ct0`, and keyword-only `per_account`, `per_search`,
  `timeout`. Note that `source_platform` is `"twitter"` (not `"x"`).
