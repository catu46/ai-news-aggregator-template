...
- `_deliver_bucket()` ranks WITHIN the bucket by adding signals into each card's score:
  ...
  - **stars** (repos only): `STAR_WEIGHT · log10(stars)` from the github `metadata` — a higher-starred repo ranks higher (log scale so a giant doesn't steamroll affinity/focus). Other platforms have no `stars`, so it's a no-op there.
