"""Seed / cold-start: loads examples of the user's taste BEFORE any real
...
For each user present in seeds.yaml:
  ...
  4. embed the text and write the embedding (feeds RECALL and the centroid);
  5. record a preloaded vote: gold => +1 (👍), noise => -1 (👎),
     with origin='seed'.

Seeds do NOT go through the curator: the verdict is left NULL on purpose (they
are not "approved/rejected posts" — they are taste labels). That's why they feed
the recall archive and the affinity prior, but don't enter the delivery flow.

Note: the CHECK on `posts.source_platform` in `db/schema.sql` ALREADY accepts
'seed' (alongside 'github' and 'manual'), so seeding upserts work directly —
there's no extra schema step.
...
def _seed_source_id(text: str) -> str:
    """Deterministic source_id => re-running the seed doesn't duplicate posts."""
...
async def _seed_user(
    ...
    """Seed all of a single user's examples."""
    ...
    for ex in examples:
        ...
        if post_id is None:
            # Already existed (re-run, or the same text repeated): embedding and
            # vote were already written on the 1st pass. With no lookup method in
            # the shared interface, we treat this as an idempotent no-op.
            result.skipped_dup += 1
            continue

        # Embedding (feeds centroid + recall). embed_documents works in batches;
        # here it's 1 at a time to keep the post_id <-> vector mapping trivial.
        [embedding] = await embedder.embed_documents([ex.text])
        await db.set_embedding(post_id, embedding, _model_of(embedder))

        # Preloaded vote: gold liked, noise not-liked. origin='seed'.
...
def _model_of(embedder: Embedder) -> str:
    """Read the Embedder's model id without relying on a nonexistent public attribute.

    The Embedder keeps the model in `_model`; we use getattr so it doesn't break
