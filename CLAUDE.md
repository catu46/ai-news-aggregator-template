...
sources (config/sources.yaml, per user) + active /focus topics
   Reddit (.rss) · GitHub (Search API + README) · X (twitter-cli via cookies)
        │   (/focus injects topics: news → Reddit+X · repos → GitHub)
   ...
   SHARED POOL: `posts` table (curated once = QUALITY verdict, user-agnostic)
        ...
        └── CURATE (Haiku 4.5 — or Kimi via CURATOR_PROVIDER — → Verdict: approve/reject + category + summary + rationale)
                    verdict IS NULL, batches of 100; aware of the /focus topics (interests loosen the bar);
   ...
   Telegram: cards with 👍/👎 → on_vote writes to `votes`
        ...
        ├── FOCUS         (/focus by speech: re-ranks delivery AND injects the topic into INGESTION)
        ...
        │
   RECALL / MCP  →  search_pool · recall_voted · active_focus  (the same methods serve /search, chat and MCP)
...
- Telegram commands (`/start`, `/feed`, `/run`, `/search`, `/focus`), card formatting, votes, 2-bucket delivery ranking, freshness cutoff (30d), auto-balancing, digest time (`DIGEST_HOUR`) and the two jobs → `src/bot/bot.py`.
