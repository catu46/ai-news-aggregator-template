...
A **personal** AI news and repositories aggregator, designed as a **multi-tenant open-source template**: each person spins up their own copy (your bot, your database, your data — nothing is shared between instances). It collects from **GitHub / Reddit / X**, curates the **quality** of each item with **Claude Haiku 4.5**, and delivers **once a day on Telegram** split into **2 buckets** — 📦 repos (GitHub) and 🗞️ news (Reddit + X) — with cards that have 👍/👎 buttons. Your votes train the ranking, and the curated archive is also accessible to Claude through an **MCP server**. It runs comfortably on free tiers (~$0–10/mo); the only variable cost is curation, which is capped by design.

## Architecture / flow

Everything runs **inside the bot itself** (always-on process): two jobs on python-telegram-bot's `JobQueue` — periodic delivery and periodic pipeline. **No separate cron needed.**

```
sources (config/sources.yaml, per user)
   ...
   SHARED POOL: `posts` table (curated once = QUALITY verdict, user-agnostic)
        ...
        └── CURATE (Haiku 4.5 → Verdict: approve/reject + category + summary + rationale)
                    verdict IS NULL, batches of 100, with a SpendGuard that pauses when the cap is hit
        │
   DELIVERY IN 2 BUCKETS (delivery job or /feed)
   ...
   Telegram: cards with 👍/👎 → on_vote writes to `votes`
        ├── AFFINITY  (ranks, separate PER BUCKET; only ranks, never hides)
        ├── FOCUS     (/foco by speech: re-ranks delivery AND injects the topic into COLLECTION)
        └── BALANCE   (by speech: mixes new × relevant; saved in the user's settings)
        │
   RECALL / MCP  →  search_pool · recall_voted · active_focus  (the same methods serve /buscar, chat, and the MCP)
...
- `config/` — `sources.yaml` (sources per user) and `seeds.yaml` (cold-start); `*.example.yaml` are versioned. See `config/CLAUDE.md`.
...
- **Curator and embedder are swappable.** The curator is a `Curator` ABC with `async classify(post_text, similarity_signal=None) -> Verdict | None`; `AnthropicCurator` is the current impl and there's a `DeepSeekCurator` sketch showing how to plug in another provider. The embedder is a thin wrapper around Voyage. The pipeline reads the model name via `curator_model_of()` / `getattr(embedder, "_model", ...)` precisely to stay decoupled.
- **Multi-tenant by `telegram_user_id`.** `users.telegram_user_id` is UNIQUE; everything is scoped by a `user_id` resolved with `get_or_create_user`. `posts` is a **shared pool**, curated **once** (quality, user-agnostic) → adding people does **not** multiply the curation cost. Each person's **taste** lives in `votes`, `deliveries`, and `focus`. Affinity is separate **per bucket** (repos vs news).
- **Idempotent steps.** Each pipeline step only touches what's pending (`embedding IS NULL`, `verdict IS NULL`); dedup happens at the upsert. A source failure is isolated and logged, it doesn't bring down the cycle.
...
- **Isolated pipeline:** `python -m src.pipeline` — runs 1 collect→embed→curate cycle, without delivering to Telegram. Good for testing ingestion.
- **Full bot:** `python -m src.bot.bot` — brings up the bot in long-polling with the 2 jobs (delivery + pipeline) embedded. It's the `startCommand` of the `Procfile`/`railway.json`.
...
- Add/adjust a collection source → `src/ingestion/` (`reddit_source.py`, `github_source.py`, `x_source.py`); the contract is `src/ingestion/base.py` (`IngestionSource.fetch()`).
- Cycle orchestration (ingest/embed/curate order, batches) → `src/pipeline.py`.
- Swap the curator or tweak the quality rubric/budget → `src/curation/curator.py` (`AnthropicCurator`, `SpendGuard`, `BudgetExceeded`); the prompt in `src/curation/prompt.py`.
...
- Telegram commands (`/start`, `/feed`, `/buscar`, `/foco`), card formatting, votes, delivery ranking, jobs → `src/bot/bot.py`.
